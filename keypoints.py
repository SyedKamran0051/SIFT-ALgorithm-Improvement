...
def get_candidate_keypoints(D, w=16):  # obtain the initial candidate keypoints
	candidates = []
...
def localize_keypoint(D, x, y, s):  #We need to compute both the Jacobian and Hessian around each candidate keypoint in order to get the offset
	dx = (D[y,x+1,s]-D[y,x-1,s])/2.  # the first derivatives are computed for the Jacobian
	dy = (D[y+1,x,s]-D[y-1,x,s])/2.  #we compute the second derivatives for the Hessian
	ds = (D[y,x,s+1]-D[y,x,s-1])/2.

	dxx = D[y,x+1,s]-2*D[y,x,s]+D[y,x-1,s]  #Discarding Low-Contrast Keypoints
